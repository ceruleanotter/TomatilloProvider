...
public class ApplicationTest extends ApplicationTestCase<Application> {
    ...
    public void testUpdateOneEntry() {
        ...
        value.put(Movie.RATING, 1);

        int rows = mContext.getContentResolver().update(
                ContentUris.withAppendedId(Movie.CONTENT_URI, id),
                value, null, null);
        assertEquals(rows, 1);
        assertCorrectStoredValues(ContentUris.withAppendedId(Movie.CONTENT_URI, id), value);
    }
    ...
    public void testUpdateMultipleEntries() {
        ...
        ContentValues value = new ContentValues();
        int updatedRating = 1;
        value.put(Movie.RATING, updatedRating);

        // Put the values in updatedRating into every rating field of Movies.
        int rows = mContext.getContentResolver().update(
                Movie.CONTENT_URI, value, null, null);
     ...
     */
    public ContentValues createDummyDataOneMovie(String title, int rating) {
        ContentValues values = new ContentValues();
